...
(function init() {
  ...
  // Clear results — wipe localStorage AND SheetDB rows for this user
  document.getElementById('resetResults').addEventListener('click', async () => {
    if (!confirm('Clear all your saved results? This cannot be undone.')) return;

    // 1. Wipe localStorage
    localStorage.removeItem(RESULTS_KEY);

    // 2. Delete this user's rows from SheetDB
    try {
      const cur = JSON.parse(localStorage.getItem('sq.currentUser') || 'null');
      const nickname = (cur && cur.nickname) || localStorage.getItem('nickname');
      if (nickname) {
        await fetch(
          `https://sheetdb.io/api/v1/p4j2z8q7vlhdc/nickname/${encodeURIComponent(nickname)}`,
          { method: 'DELETE' }
        );
        console.log('✅ SheetDB rows deleted for:', nickname);
      }
    } catch (err) {
      console.warn('⚠️ Could not delete SheetDB rows:', err);
    }
