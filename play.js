...
function savePlayState(state = {}) {
  const rec = { level, idx, score, started, ...state, at: Date.now(), nickname };
...
function loadPlayState() {
  try {
    ...
    // Reject state that belongs to a different user
    if (obj && obj.nickname && obj.nickname !== nickname) return null;
...
function showLevelEnd() {
  ...
  // Save with started:false, idx:0, score:0 so if the user navigates away
  // and comes back, the level starts cleanly from question 1
  savePlayState({ started: false, idx: 0, score: 0 });
}
...
(function initFromState() {
  ...
  if (!hasDeepLink) {
    ...
    if (ps) {
      ...
      // Only restore idx/score if the player was actively mid-game.
      // If started=false (they left from the start card or level-end panel),
      // reset idx and score so the level starts cleanly from question 1.
      if (ps.started) {
        idx = ps.idx ?? 0;
        score = ps.score ?? 0;
      } else {
...
// ── Save score to SheetDB (upsert via composite key nickname_level) ──────────
// Uses a "key" column = nickname + "_" + level so we can target exactly one row.
// Add a column named "key" to your scores Google Sheet if not already present.
...
async function updateUserScore(nicknameVal, levelNum, scoreNum) {
  ...
  const rowKey = `${nicknameVal}_${levelNum}`;  // e.g. "Alice_3"

  try {
    // Step 1: Check if a row already exists for this exact nickname+level
    const searchRes = await fetch(
      `${SCORES_API}/search?key=${encodeURIComponent(rowKey)}`
    );
    const existing = await searchRes.json();

    if (existing && existing.length > 0) {
      // Row exists → PATCH score and date only
      await fetch(
        `${SCORES_API}/key/${encodeURIComponent(rowKey)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ data: { score: String(scoreNum), date } })
        }
      );
      console.log(`✅ Score updated for ${rowKey}: ${scoreNum}`);
    } else {
      // No row yet → POST a new row
      ...
      console.log(`✅ Score created for ${rowKey}: ${scoreNum}`);
    }
  } catch (err) {
